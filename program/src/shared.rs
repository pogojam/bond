...
use {
	...
	std::{collections::BTreeSet, error::Error},
    };
    
    /// Initialization flag size for account state
    pub const INITIALIZED_BYTES: usize = 1;
    /// Storage for the serialized size of the BTreeSet control
    pub const BTREE_LENGTH: usize = 4;
    /// Storage for the serialized BTreeSet container
    ...
    pub fn unpack_from_slice(src: &[u8]) -> Result<(bool, BTreeSet<Pubkey>), Box<dyn Error>> {
	...
	if data_len == 0 {
	    Ok((is_initialized, BTreeSet::<Pubkey>::new()))
	} else {
	    let data_dser = BTreeSet::<Pubkey>::try_from_slice(&data_src[0..data_len]).unwrap();
